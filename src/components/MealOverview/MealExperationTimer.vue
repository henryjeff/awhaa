<template>
  <div>
    ...
    <ion-row style="padding-left:5px;padding-right:5px;">
      <ion-progress-bar v-if="progress_val <= 0.6" v-bind:value="progress_val" color="success"></ion-progress-bar>
      <ion-progress-bar v-else-if="progress_val <= 0.85" v-bind:value="progress_val" color="warning"></ion-progress-bar>
      <ion-progress-bar v-else v-bind:value="progress_val" color="danger"></ion-progress-bar>
    </ion-row>
    <ion-row>
      <ion-col align-self-end>
        <ion-card-subtitle  v-if="progress_val <= 0.6" class="meal-condition">Good Condition - {{time_left}} left</ion-card-subtitle>
        <ion-card-subtitle  v-else-if="progress_val <= 0.85" class="meal-condition" >Eat Soon -  {{time_left}}</ion-card-subtitle>
        <ion-card-subtitle  v-else class="meal-condition">Expires Soon {{time_left}}</ion-card-subtitle>
      </ion-col>
    </ion-row>
  </div>
</template>
...
export default {
  ...
  created() {
    ...
    this.progress_val = 1 - (diff_n_b / diff_c_b)
